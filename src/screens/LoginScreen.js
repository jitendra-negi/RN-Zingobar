...
import config, { AppFunctions } from "../common/config";
...
GoogleSignin.configure({
  webClientId:
    "12345789-g2folaog0ck90vhbvbooqthq397qvk5p.apps.googleusercontent.com",
...
function LoginScreen(props) {
  ...
  const loginWithPhoneNo = () => {
    if (validateMobile()) {
      ...
      try {
        getApi.postData("user/loginUsingMobile", sendData).then((response) => {
          ...
          if (response.status == 1) {
            ...
            props.navigation.push("VerifyMobileOTPScreen", {
              mobile: mobileNumber,
            });
          }
  ...
  const login = () => {
    if (validate()) {
      ...
      try {
        getApi.postData("user/login", sendData).then((response) => {
          logfunction("RESPONSE ", response);
          if (response.status == 1) {
            logfunction("RESPONSE ", "Success");
            setData({
  ...
  const sendOtp = async () => {
    if (validateMobile()) {
      ...
      try {
        getApi
          .postData("user/loginverifyOTP", sendData)
          .then(async (response) => {
            ...
            if (response.status == 1) {
              ...
              props.navigation.push("VerifyOTPScreen", {
                VERIFICATION_DATA: confirmation,
              });
            } else {
              setData({
                ...formData,
          ...
          <View style={styles.registerView}>
            ...
            >
              <Text style={styles.signupTxt}>
                {" "}
                {strings.login.button_registration}{" "}
              </Text>
            </TouchableOpacity>
          </View>

          {AppFunctions.socialLogin && <OtrixDivider size={"md"} />}
          {AppFunctions.socialLogin && (
            <OtrixSocialContainer
