...
function OrderDetailScreen(props) {
  ...
  const submitCancelOrder = () => {
    ...
    if (cancelValidate()) {
      ...
      //login to our server 🧛🏻‍♀️
      try {
        getApi.postData("user/cancelOrder", sendData).then((response) => {
          console.log("CancleResponse", response);
          logfunction("Social RESPONSE ", response);
          if (response.status == 1) {
            logfunction("RESPONSE ", "Success");
            setState({
  ...
  useEffect(() => {
    ...
    try {
      getApi
        .getData("user/getOrderTimeline/" + orderData.id)
        .then((response) => {
          console.log("OrderTimeLine Response", response);
          if (response.status == 1) {
            setState({
              ...state,
    ...
    <OtrixContainer customStyles={{ backgroundColor: Colors().light_white }}>
      ...
      <OtrixContent style={styles.addressContent}>
        ...
        >
          <Text style={styles.deliveryTitle}>
            {strings.order_details.view_order_detail}
          </Text>
          ...
          <View style={styles.cartContent}>
            <View style={[styles.detailBox, { height: hp("15%") }]}>
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={styles.leftTxt}>
                    {strings.order_details.order_date}
                  </Text>
                </View>
              ...
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={styles.leftTxt}>
                    {strings.order_details.payment_method}
                  </Text>
                </View>
                <View style={styles.rightView}>
              ...
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={[styles.leftTxt]}>
                    {strings.order_details.order_total}
                  </Text>
                </View>
                <View style={styles.rightView}>
          ...
          <OtrixDivider size={"md"} />
          <Text style={styles.deliveryTitle}>
            {strings.order_details.shipping_address}
          </Text>
          <OtrixDivider size={"sm"} />
          <View style={styles.cartContent}>
          ...
          <OtrixDivider size={"md"} />
          <Text style={styles.deliveryTitle}>
            {strings.order_details.order_product_list}
          </Text>
          <OtrixDivider size={"sm"} />
                ...
                <View>
                  <View style={styles.cartBox}>
                    ...
                    <View style={styles.infromationView}>
                      ...
                      <Text style={styles.orderDate}>
                        {strings.order_details.quantity}: {item.quantity}
                      </Text>
                    </View>
                  ...
                  >
                    <Text style={styles.bottomLeftTxt}>
                      {" "}
                      {strings.order_details.rate_this_product}{" "}
                    </Text>
                    <Icon
          ...
          <OtrixDivider size={"md"} />
          <Text style={styles.deliveryTitle}>
            {strings.order_details.order_summary}
          </Text>
          <OtrixDivider size={"sm"} />
          <View style={styles.cartContent}>
            <View style={[styles.detailBox, { height: hp("22%") }]}>
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={styles.leftTxt}>
                    {strings.order_details.items_count}
                  </Text>
                </View>
                <View style={styles.rightView}>
                  <Text style={styles.rightTxt}>
              ...
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={styles.leftTxt}>
                    {strings.order_details.discount}
                  </Text>
                </View>
                <View style={styles.rightView}>
                  <Text style={styles.rightTxt}>
              ...
              <View style={styles.detailRow}>
                <View style={styles.leftView}>
                  <Text style={styles.leftTxt}>
                    {strings.order_details.shipping_charge}
                  </Text>
                </View>
                <View style={styles.rightView}>
                  <Text style={styles.rightTxt}>
      ...
      {orderData.order_status.name === "Pending" ? (
        <Button
          ...
          style={[
            GlobalStyles.button,
            { marginHorizontal: wp("2%"), marginVertical: wp("2%") },
          ]}
          onPress={() =>
            setState({ ...state, orderID: orderData.id, cancelOrder: true })
          }
        >
          <Text style={GlobalStyles.buttonText}>
            {strings.order_details.cancel_order}
          </Text>
        </Button>
      ) : null}
          ...
          <View style={styles.modelView}>
            ...
            <View style={styles.contentView}>
              ...
              <Text style={styles.rateTitle}>
                {strings.order_details.rate_product_now}!
              </Text>
              ...
              <FormControl isRequired isInvalid={submited && "name" in errors}>
                <Input
                  variant="outline"
                  value={name}
                  placeholder={strings.order_details.rate_text}
        ...
        <View>
          ...
          <View style={styles.modelView}>
            ...
            <View style={styles.contentView}>
              ...
              <Text style={styles.rateTitle}>
                {strings.order_details.cancel_order_text}!
              </Text>

              <OtrixDivider size={"md"} />
              <FormControl
                isRequired
                isInvalid={cancelSubmited && "cancelMessage" in errors}
              >
                <Input
                  variant="outline"
                  value={cancelMessage}
                  placeholder={strings.order_details.cancel_text}
                  style={GlobalStyles.textInputStyle}
                  onChangeText={(value) => {
                    setState({
                      ...state,
